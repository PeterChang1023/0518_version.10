...
function Compute_Corrections!(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  grid_t_n::Array{Float64, 3}, spe_t_n::Array{ComplexF64, 3}, grid_tracers_p::Array{Float64, 3}, grid_tracers_c::Array{Float64, 3}, grid_tracers_n::Array{Float64, 3},grid_δtracers::Array{Float64,3}, grid_tracers_all::Array{Float64,3},sum_tracers_p::Float64, spe_tracers_n::Array{ComplexF64, 3}, grid_t::Array{Float64, 3})
  ...
  if (do_water_correction) 
    ...
    tracers_ratio   =  grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, (grid_tracers_p[:,:,:] - grid_tracers_n[:,:,:]), grid_ps_n) .* (es[:,:,:] ./ (grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, es, grid_ps_n)))
    ...
    tracers_ratio   = grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, (grid_tracers_p[:,:,:] - grid_tracers_n[:,:,:]), grid_ps_n) .* (es[:,:,:] ./ (grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, es, grid_ps_n)))
    # check
    @info (sum_tracers_p - sum_tracers_n) maximum(tracers_ratio)
...
function Spectral_Dynamics!(mesh::Spectral_Spherical_Mesh,  vert_coord::Vert_Coordinate, 
  ...
  grid_t_n, spe_t_n, grid_tracers_p, grid_tracers_c, grid_tracers_n, grid_δtracers, grid_tracers_full, sum_tracers_p, spe_tracers_n, grid_t)
