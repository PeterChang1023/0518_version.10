...
function Atmos_Spectral_Dynamics_Main(physcis_params::Dict{String, Float64}, end_day::Int64 = 3, spinup_day::Int64 = 0)
  ...
  num_grid_tracters = 1
  num_spe_tracters  = 1
  dyn_data = Dyn_Data(name, num_fourier, num_spherical, nλ, nθ, nd) ### origin = Dyn_Data(name, num_fourier, num_spherical, nλ, nθ, nd)
  ...
  for i = 2:NT
    ...
    if (integrator.time%day_to_sec == 0)
      @info "Day: ", div(integrator.time,day_to_sec), " Max |U|,|V|,|P|,|T|, |e| : ", maximum(abs.(dyn_data.grid_u_c)), maximum(abs.(dyn_data.grid_v_c)), maximum(dyn_data.grid_p_full), maximum(dyn_data.grid_t_c), maximum(dyn_data.grid_tracers_c)
    end
