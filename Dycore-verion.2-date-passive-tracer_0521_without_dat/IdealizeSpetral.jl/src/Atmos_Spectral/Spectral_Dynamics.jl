...
function Compute_Corrections!(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  grid_t_n::Array{Float64, 3}, spe_t_n::Array{ComplexF64, 3}, grid_tracers_p::Array{Float64, 3}, grid_tracers_c::Array{Float64, 3}, grid_tracers_n::Array{Float64, 3},grid_δtracers::Array{Float64,3}, grid_tracers_all::Array{Float64,3},sum_tracers_p::Float64, spe_tracers_n::Array{ComplexF64, 3}, grid_t::Array{Float64, 3}, grid_ps_p::Array{Float64, 3}, Δt::Int64, grid_p_full::Array{Float64, 3})
  ...
  if (do_water_correction) 
    ...
    if do_muti
        ...
        q_sum = Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, (grid_tracers_p-grid_tracers_n) ./1000 .*0.622 ./ grid_p_full, grid_ps_n) * 9.81 * 128*64*20
        es .= 611*exp.(Lv/Rv*(one_array[:,:,:] ./ new_array[:,:,:]-one_array[:,:,:] ./ grid_t[:,:,:])) 
        ...
        grid_tracers_n[:,:,:] .+= (es[:,:,:]/es_sum * q_sum).*grid_p_full[:,:,:] ./0.622
    ...
    if do_minus
        ...
        # do qv correction factor
        tracers_ratio   = sum_tracers_p - (sum_tracers_n)
        #reshape_e       = reshape(grid_tracers_n, 128*64*20,1)
        #max_e           = sort(reshape_e, dims=1, rev=true) 
        if tracers_ratio >= 0 
            grid_tracers_n[:,:,1:15] .+= tracers_ratio / (128*64*15)
            # After correction, do the new sum of qv and tracers_ratio at next
            grid_tracers_all .= grid_tracers_n 
            sum_tracers_n    = grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, grid_tracers_all, grid_ps_n)
            tracers_ratio   = sum_tracers_p - (sum_tracers_n)
            # check
            @info "minus method:" (sum_tracers_p - sum_tracers_n) (tracers_ratio)
        end
        
        if tracers_ratio < 0
            do_adjust = zeros(size(grid_tracers_n))
            
            g = 0
            for i in range(1,128)
                for j in range(1,64)
                    for k in range(1,20)
                        if grid_tracers_n[i,j,k] > 0
                            g += 1
                        end
                    end
                end
            ...
            for i in range(1,128)
                for j in range(1,64)
                    for k in range(1,20)
                        if (grid_tracers_n[i,j,k] + tracers_ratio / g) >= 0
                            grid_tracers_n[i,j,k] += tracers_ratio / g
                        else
                            break
                        end
                        #@assert ("do muti")
...
function Spectral_Dynamics!(mesh::Spectral_Spherical_Mesh,  vert_coord::Vert_Coordinate, 
  ...
  ### By CJY 0521 ####
  HS_forcing_water_vapor!(grid_tracers_c,  grid_δtracers, grid_t, grid_δt, grid_p_full, grid_tracers_p)
  ...
  grid_t_n, spe_t_n, grid_tracers_p, grid_tracers_c, grid_tracers_n, grid_δtracers, grid_tracers_full, sum_tracers_p, spe_tracers_n, grid_t, grid_ps_p, Δt, grid_p_full)
...
"""
  ...
  new_array .= 273.15
  #for k = nd-2:nd
  grid_tracers_c[:,:,:] .= 611*exp.(Lv/Rv*(one_array[:,:,:] ./ new_array[:,:,:]-one_array[:,:,:] ./ grid_t[:,:,:])) * initial_RH
  #@info maximum.(grid_tracers_c)
  #end
  # By CJY2
...
function HS_forcing_water_vapor!(grid_tracers_c::Array{Float64, 3}, grid_δtracers::Array{Float64, 3}, grid_t::Array{Float64, 3}, grid_δt::Array{Float64, 3}, grid_p_full::Array{Float64, 3},grid_tracers_p::Array{Float64, 3})
  es  = zeros(size(grid_tracers_c))
  extra_e = zeros(size(grid_tracers_c))
  extra_e_p = zeros(size(grid_tracers_c)) 
  ...
  new_array .= 273.15
  initial_RH  = 1.0
  es[:,:,:] .= 611*exp.(Lv/Rv*(one_array[:,:,:] ./ new_array[:,:,:]-one_array[:,:,:] ./ grid_t[:,:,:])) * initial_RH
  for k = 1:20
        for j=1:64
            for i=1:128
                ...
                elseif grid_tracers_p[i,j,k] >= es[i,j,k]
                    extra_e_p[i,j,k] = grid_tracers_p[i,j,k] - es[i,j,k]
                    grid_tracers_p[i,j,k] = es[i,j,k]
                else
                    grid_tracers_c[i,j,k] = grid_tracers_c[i,j,k] 
                    grid_tracers_p[i,j,k] = grid_tracers_p[i,j,k]
                end
            end
        end
  grid_δt[:,:,k] .+= ((extra_e[:,:,k] ./1000  .*0.622 ./(grid_p_full[:,:,k]) .* Lv ./ cp) ./ day_to_sec) ./40.
  end
end
